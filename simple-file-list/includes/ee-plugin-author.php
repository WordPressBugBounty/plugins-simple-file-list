...
<article>
	...
	<h1>' . __('Thank You', 'simple-file-list') . '</h1>

	<p>' . __('Thank you for using my plugin. I am proud of this work and am committed to supporting it.', 'simple-file-list') . ' ' . __('The goal is to keep it simple, yet make it do what you need it to do.', 'simple-file-list') . ' ' . __('Tell me about the features that you want.', 'simple-file-list') . ' </p>


	<p><a href="http://mitchellbennis.com/" target="_blank">Mitchell Bennis</a><br />
	Cokato, Minnesota, USA</p>'; // That's me!

		$eeOutput .= '<p>' . __('Contact Me', 'simple-file-list') . ': <a href="https://simplefilelist.com/support/">' . __('Feedback or Questions', 'simple-file-list') . '</a></p>';

	if(is_array($eeContributors)) {

		$eeOutput .= '<hr />

		<h3>' . __('Contributors', 'simple-file-list') . '</h3>
		...
		foreach( $eeContributors as $eeValue){
			...
			$eeOutput .= __('Thanks to', 'simple-file-list') . ' <a href="' . $eeArray[1] . '" target="_blank">' . @$eeArray[0] . ' </a>' . @$eeArray[2] . '<br />';
		}
