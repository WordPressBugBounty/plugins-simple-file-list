...
if( isset($_POST['eePost']) AND check_admin_referer( 'ee-simple-file-list-settings', 'ee-simple-file-list-settings-nonce')) {
	...
	$eeSFL_BASE->eeLog[eeSFL_BASE_Go]['messages'][] = __('List Settings Saved', 'simple-file-list');
...
<div class="eeColFull eeSettingsTile">
	...
	<h2>' . __('List Location', 'simple-file-list') . '</h2>
		...
		<div class="eeNote">' . __('Upgrade to Pro', 'simple-file-list') . '</a> &rarr; ' . __('The Pro Version allows you to define a custom file list directory. It must only be relative to the WordPress home directory.', 'simple-file-list') . '</div>
...
<div class="eeColumns">
	...
	<div class="eeColLeft">
		...
		<fieldset>
			...
			<div class="eeNote"><a href="https://get.simplefilelist.com/" target="_blank">' .  __('Upgrade to PRO', 'simple-file-list') . '</a> ' . __('Upgrade to allow greater file management control for specific users and roles.', 'simple-file-list') . '<br />
			</div>
		...
		if( !in_array('ImageMagick' , $eeSupported) ) {
			$eeMissing[] = __('Image Magick is Not Installed. PDF thumbnails cannot be created.', 'simple-file-list');
		}

		if( !in_array('Shell' , $eeSupported) ) {

			$eeMissing[] = __('Shell Access is Denied. PDF and Video thumbnails cannot be created.', 'simple-file-list');
			$eeMissing[] = __('GhostScript is not available. PDF thumbnails cannot be created.', 'simple-file-list');

		} else {

			if( !in_array('GhostScript' , $eeSupported) ) {
				$eeMissing[] = __('GhostScript is Not Installed. PDF thumbnails cannot be created.', 'simple-file-list');
			}
		...
		$eeOutput .= '</p>
		<div class="eeNote">' . __('Read a PDF file and create a representative thumbnail image based on the first page.', 'simple-file-list');
		...
		if( !in_array('ffMpeg' , $eeSupported) OR !in_array('Shell' , $eeSupported) ) {
			$eeMissing[] = __('Video thumbnails will not be created because ffMpeg is not available.', 'simple-file-list');
		}
	...
	<div class="eeColRight">
		...
		<div><label>' . __('Show Link', 'simple-file-list') . '</label>
